...
fn apply_diff(screen: &mut ScreenContent, msg: ServerMsg) {
    match msg {
        ServerMsg::ScreenDiff {
            ...
            screen.scroll_offset = scroll_offset;
            screen.scrollback_len = scrollback_len;
        }
...
#[test]
fn scroll_metadata_propagates_through_diff() {
    let a = make_screen(&["hello"]);
    let mut b = a.clone();
    b.scroll_offset = 3;
    b.scrollback_len = 20;
    ...
    assert_eq!(cached.scroll_offset, 3);
    assert_eq!(cached.scrollback_len, 20);
}
