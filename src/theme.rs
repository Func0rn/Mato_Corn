...
pub fn builtin(name: &str) -> ThemeColors {
    match name {
        "corn" => ThemeColors {
            ...
            bg: [231, 214, 151],      // toasted corn silk, darker than white
            surface: [221, 202, 133], // warm husk panel
            border: [166, 130, 58],   // dried husk gold-brown
            accent: [185, 132, 31],   // roasted corn yellow
            accent2: [111, 150, 82],  // pale leaf green
            fg: [68, 42, 18],         // corn-silk brown text
            fg_dim: [112, 81, 43],    // muted tassel brown
            sel_bg: [202, 174, 93],   // readable soft yellow selection
        },
        "system" => ThemeColors {
...
pub fn load() -> ThemeColors {
    ...
    let base_name = file.name.as_deref().unwrap_or("corn");
...
pub fn selected_name() -> String {
    ...
    let file: ThemeFile = std::fs::read_to_string(&path)
        ...
        .and_then(|s| toml::from_str(&s).ok())
        .unwrap_or_default();
    file.name.unwrap_or_else(|| "corn".to_string())
}
