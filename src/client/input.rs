use crate::client::app::{
    App, Focus, JumpMode, OfficeDeleteConfirm, PresetField, RenameState, RenameTarget,
...
fn is_text_backspace(key: &crossterm::event::KeyEvent) -> bool {
    let ctrl = key.modifiers.contains(KeyModifiers::CONTROL);
    matches!(key.code, KeyCode::Backspace | KeyCode::Delete)
        || matches!(key.code, KeyCode::Char('\u{7f}'))
        || (ctrl && matches!(key.code, KeyCode::Char('h') | KeyCode::Char('H')))
}
...
pub fn handle_key(app: &mut App, key: crossterm::event::KeyEvent) -> bool {
    ...
    if key.code == KeyCode::F(2) && app.new_desk.is_none() && !app.terminal_preset_state.active {
        app.open_terminal_presets();
        return false;
    }

    if key.code == KeyCode::F(3) {
        if app.mouse_select_mode || app.copy_mode {
            app.mouse_select_mode = false;
            app.copy_mode = false;
            app.show_toast("Mato mouse mode restored");
        } else {
            app.focus = Focus::Content;
            app.copy_mode = true;
            app.mouse_select_mode = true;
            app.show_toast("Fullscreen select: drag terminal text, F3/Esc to return");
        }
        return false;
    }

    if app.mouse_select_mode {
    ...
    if app.new_desk.is_some() {
        if is_text_backspace(&key) {
            app.new_desk_backspace();
            return false;
        }
        match key.code {
            KeyCode::Enter => app.commit_new_desk(),
            KeyCode::Esc => app.cancel_new_desk(),
            KeyCode::Tab => app.new_desk_complete(),
            KeyCode::Up => app.new_desk_select(-1),
            KeyCode::Down => app.new_desk_select(1),
            KeyCode::Char(c) if !ctrl => app.new_desk_insert_char(c),
            _ => {}
        }
        return false;
    }
    ...
    if app.terminal_preset_state.active {
        if app.terminal_preset_state.editor.is_some() {
            let mut commit = false;
            let mut cancel = false;
            if let Some(editor) = &mut app.terminal_preset_state.editor {
                if is_text_backspace(&key) {
                    match editor.field {
                        PresetField::Name => {
                            editor.name.pop();
                ...
                match key.code {
                    ...
                    KeyCode::Enter => {
                        if editor.field == PresetField::Name {
                            editor.field = PresetField::Command;
                        } else {
                            editor.command.push('\n');
                        }
                    }
                    KeyCode::Tab | KeyCode::BackTab => {
                        editor.field = match editor.field {
                            PresetField::Name => PresetField::Command,
                            PresetField::Command => PresetField::Name,
                        };
                    }
                    KeyCode::Char(c) if !ctrl => match editor.field {
                        PresetField::Name => editor.name.push(c),
                        PresetField::Command => editor.command.push(c),
                    },
                    _ => {}
                }
            }
            if cancel {
                app.cancel_preset_editor();
            }
            if commit {
                app.commit_preset_editor();
            }
            app.dirty = true;
            return false;
        }
        ...
        match key.code {
            KeyCode::Esc | KeyCode::Char('q') => app.close_terminal_presets(),
            KeyCode::Up => app.select_terminal_preset(-1),
            KeyCode::Down => app.select_terminal_preset(1),
            KeyCode::Enter => app.set_selected_preset_as_default(),
            KeyCode::Char('m') => {
                app.terminal_preset_state.manage = !app.terminal_preset_state.manage;
                app.dirty = true;
            }
            KeyCode::Char('a') => app.begin_new_preset(),
            KeyCode::Char('e' | 'r') => app.begin_edit_preset(),
            KeyCode::Char('d' | 'x') if app.terminal_preset_state.manage => {
    ...
    match app.focus {
        Focus::Sidebar => match key.code {
            ...
            KeyCode::Char('o') => app.show_toast("Office layer is disabled; use Desks"),
        ...
        Focus::Topbar => match key.code {
            ...
            KeyCode::Char('p') => app.open_terminal_presets(),
            KeyCode::Char('x') => {
