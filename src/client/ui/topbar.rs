...
pub(super) fn draw_topbar(f: &mut Frame, app: &mut App, area: Rect, t: &ThemeColors) {
    ...
    for i in app.tab_scroll..task.tabs.len() {
        ...
        let style = if show_alarm {
            if t.follow_terminal {
                if app.alarm_pulse_on() {
                    Style::default().add_modifier(Modifier::BOLD | Modifier::REVERSED)
                } else {
                    Style::default().add_modifier(Modifier::BOLD)
                }
            } else if app.alarm_pulse_on() {
                Style::default()
                    .fg(t.bg())
                    .bg(t.accent2())
...
pub(super) fn draw_statusbar(f: &mut Frame, app: &App, area: Rect, t: &ThemeColors) {
    ...
    let keys: &[(&str, &str)] = if app.mouse_select_mode {
        &[
            ("Drag", "Host Select"),
            ("F3/Esc", "Mato Mouse"),
        ]
    } else if app.copy_mode {
        &[
            ("↑/k", "Scroll Up"),
            ("↓/j", "Scroll Down"),
    ...
    } else if app.new_desk.is_some() {
        &[("Enter", "Create/Open"), ("Tab", "Complete"), ("Esc", "Cancel")]
    } else if app.terminal_preset_state.active {
        if app.terminal_preset_state.editor.is_some() {
            &[("Enter", "Next/Save"), ("Tab", "Field"), ("Esc", "Cancel")]
        } else if app.terminal_preset_state.manage {
            &[("↑↓", "Select"), ("Enter", "Set Default"), ("a/e/d", "Edit"), ("m", "Back")]
        } else {
            &[("↑↓", "Select"), ("Enter", "Set Default"), ("m", "Manage"), ("Esc", "Close")]
        }
