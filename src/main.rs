...
fn main() -> Result<()> {
    ...
    // First run now starts with the Desk/Terminal model directly. Office templates
        ...
        desks: new_office
            ...
            .map(|d| {
                let cwd = d.cwd.or_else(|| {
                    Some(
                        client::presets::desk_path_for_name(&d.name)
                            .to_string_lossy()
                            .to_string(),
                    )
                });
                let tabs = d
                    .tabs
                    .into_iter()
                    .map(|tb| TabEntry::with_saved(tb.id, tb.name, cwd.clone(), None))
                    .collect();
                Desk {
                    id: d.id,
                    name: d.name,
...
fn run_client() -> Result<()> {
    ...
    loop {
        ...
        match &mut screen {
            ScreenState::Main => {
                // Mouse focus switching requires crossterm mouse capture. Disable it
                // only in Copy Mode so the host terminal can own drag selection.
                let desired_mouse_capture = !app.copy_mode && !app.mouse_select_mode;
                ...
                let ui_changed = app.dirty
                    ...
                    || app.alarm_needs_update()
                    || (!app.copy_mode && !app.mouse_select_mode && current_gen != app.last_rendered_screen_gen);
