...
impl DaemonProvider {
    ...
    pub fn set_spawn_init_command(&mut self, command: Option<String>) {
        self.spawn_init_command = command.filter(|s| !s.trim().is_empty());
    }
...
impl TerminalProvider for DaemonProvider {
    fn spawn(&mut self, rows: u16, cols: u16) {
        ...
        let resp = self.send_msg(ClientMsg::Spawn {
            tab_id: self.tab_id.clone(),
            rows,
            cols,
